...
class wx_user(models.Model):
    ...
    subscribe_time = fields.Datetime(u'关注时间', )
    ...
    @api.model
    def sync(self):
        if not self.env.user.has_group('oejia_wx.group_social_network_manager'):
            raise ValidationError(u'无权限操作，请联系管理员！')
        # print 'no operate!'
        next_openid = 'init'
        c_total = 0
        c_flag = 0
    ...
    def _get_headimg(self):
        self.headimg = u'<img src=%s width="100px" height="100px" />' % (
    ...
    def _get_groups(self):
        ...
        objs = Group.search([])
        # return [(str(e.group_id), e.group_name + str(e.group_id)) for e in objs]
