...
class dingtalkSale(models.Model):
    ...
    def create(self, vals):
        ...
        _logger.info("钉钉发消息：重写create，定位多次发送问题所在")
        # 爱车管家为后付费，所以下单即发送消息
        ...
        if 1:
            ...
            # employees = vals.get('message_follower_ids')
            if not employees:
                return sale_order
            ...
            content = "新订单：%s，业主：%s，电话：%s，项目：%s，说明：%s, 数量：%d，地址：%s。" % ctuple
    ...
    def write(self, vals):
        ...
        for so in self:
            ...
            if so.state == "sale" and vals.get('state') and vals['state'] == 'sale':
                target_employee_ids = ""
                # 获得发消息的部门
                departments = self.env['hr.department'].search([('monitor_sale', '=', True)])
                department_ids = [depart.id for depart in departments]
